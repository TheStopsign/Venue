...
lectureRoutes.post("/update/:id", function(req, res) {
  ...
  if (lecture_id && updated_lecture) {
    Lecture.findByIdAndUpdate(lecture_id, updated_lecture, function(
      err,
      lecture
    ) {
      if (err || !lecture) {
...
lectureRoutes
  .route("/get_signed_recording_url/:filename")
  ...
  if (req.user.instructor_courses.length) {
    promises.push(
      new Promise((resolve, reject) => {
        ...
        Course.find(
          { _id: { $in: req.user.instructor_courses } },
          (error, instructor_courses) => {
            if (error || instructor_courses == null) {
              console.log(
                "<ERROR> Getting courses with instructor ID:",
                user._id
              );
              res.json(error);
            } else {
              ...
              Section.find(
                ...
                (error, instructor_sections) => {
                  if (error || instructor_sections == null) {
                    console.log(
                      "<ERROR> Getting sections for courses:",
                      instructor_courses
                    );
                    res.json(error);
                  } else {
                    ...
                    Lecture.find(
                      { sections: { $in: instructor_sections } },
                      (error, instructor_lectures) => {
                        if (error || instructor_lectures == null) {
                          console.log(
                            "<ERROR> Getting lectures for sections:",
                            instructor_sections
                          );
                          res.json(error);
                        } else {
                          console.log(
                            "<SUCCESS> Getting lectures for instructor ID: " +
                              req.user._id +
                              ", with preference: " +
                              preference
                          );
                          ...
                          if (preference === "none")
                            ...
                            let promises2 = [];
                            instructor_lectures.forEach(
                              (instructor_lecture) => {
                                for (
                                  let i = 0;
                                  i < instructor_lecture.sections.length;
                                  i++
                                ) {
                                  let lecture_section =
                                    instructor_lecture.sections[i];
                                  instructor_sections.forEach(
                                    (instructor_section) => {
                                      if (
                                        lecture_section.equals(
                                          instructor_section._id
                                        )
                                      ) {
                                        promises2.push(
                                          new Promise((resolve2, reject2) => {
                                            instructor_lecture.sections[
                                              i
                                            ] = instructor_section;
                                            if (
                                              preference ===
  ...
  if (req.user.ta_sections.length) {
    promises.push(
      new Promise((resolve, reject) => {
        // User is a ta
        Section.find(
          { _id: { $in: req.user.ta_sections } },
          (error, ta_sections) => {
            Lecture.find(
              { sections: { $in: ta_sections } },
              (error, ta_lectures) => {
                if (error || ta_sections == null) {
                  ...
                  console.log(
                    "<SUCCESS> Getting lectures for ta ID: " +
                      req.user._id +
                      ", with preference: " +
                      preference
                  );
                  ...
                  // attach sections or courses to lectures based on preference
                  if (preference === "none") resolve(ta_lectures);
                  else {
                    let course_promises = [];
                    ...
                    ta_lectures.forEach((ta_lecture) => {
                      ...
                      let lecture_sections = ta_sections.filter((section) =>
                        ta_lecture.sections.includes(section._id)
                      );
                      ta_lecture.sections = lecture_sections;
                      if (preference === "with_sections_and_course") {
                        course_promises.push(
                          new Promise((resolve2, reject2) => {
                            Course.findById(
                              lecture_sections[0].course,
                              (error, lecture_course) => {
                                if (error || lecture_course == null) {
                                  console.log(
                                    "<ERROR> Getting course for section:",
                                    lecture_section
                                  );
                                  resolve2(null);
                                } else {
                    ...
                    if (preference === "with_sections_and_course") {
                      Promise.all(course_promises).then((resolved) => {
                        lecture_courses = resolved.filter(
                          (lecture_course) => lecture_course != null
                        );
                        for (let i = 0; i < ta_lectures.length; i++) {
                          ...
                          for (let j = 0; j < ta_lecture.sections.length; j++) {
                            ...
                            lecture_courses.forEach((lecture_course) => {
                              if (
                                lecture_course._id.equals(
                                  lecture_section.course
                                )
                              )
                                ta_lectures[i].sections[
                                  j
  ...
  if (req.user.student_sections.length) {
    promises.push(
      new Promise((resolve, reject) => {
        // User is a student
        Section.find(
          { _id: { $in: req.user.student_sections } },
          (error, student_sections) => {
            Lecture.find(
              { sections: { $in: student_sections } },
              (error, student_lectures) => {
                if (error || student_sections == null) {
                  ...
                  console.log(
                    "<SUCCESS> Getting lectures for student ID: " +
                      req.user._id +
                      ", with preference: " +
                      preference
                  );
                  ...
                  // attach sections or courses to lectures based on preference
                  if (preference === "none") resolve(student_lectures);
                  else {
                    let course_promises = [];
                    // attach sections to lectures and make asnc call for courses if requested
                    student_lectures.forEach((student_lecture) => {
                      // get the actual section object for the section reference
                      let lecture_sections = student_sections.filter(
                        (section) =>
                          student_lecture.sections.includes(section._id)
                      );
                          ...
                          new Promise((resolve2, reject2) => {
                            Course.findById(
                              ...
                              (error, lecture_course) => {
                                if (error || lecture_course == null) {
                                  console.log(
                                    "<ERROR> Getting course with ID:",
                                    lecture_sections[0]._id
                                  );
                                  resolve2(null);
                                } else {
                                  resolve2(lecture_course);
                    ...
                    if (preference === "with_sections_and_course") {
                      Promise.all(course_promises).then((resolved) => {
                        ...
                        for (let i = 0; i < student_lectures.length; i++) {
                          ...
                          for (
                            let j = 0;
                            j < student_lecture.sections.length;
                            j++
                          ) {
...
lectureRoutes.get("/for_course/:course_id", (req, res) => {
  ...
  Section.find({ course: course_id }, (error, course_sections) => {
    if (error || course_sections == null) {
      ...
      Lecture.find(
        { sections: { $in: course_sections } },
        (error, course_lectures) => {
          if (error || course_lectures == null) {
            ...
            console.log(
              "<SUCCESS> Getting lectures for course with ID:",
              course_id
            );
...
lectureRoutes.get("/for_section/:section_id", (req, res) => {
  ...
  Lecture.find({ sections: section_id }, (error, section_lectures) => {
    if (error || section_lectures == null) {
      ...
      console.log(
        "<SUCCESS> Getting lectures for section with ID:",
        section_id
      );
...
lectureRoutes.get("/with_sections_and_course/:lecture_id", (req, res) => {
  ...
  Lecture.findById(lecture_id, function(error, lecture) {
    if (error || lecture == null) {
      ...
      Section.find(
        { _id: { $in: lecture.sections } },
        (error, lecture_sections) => {
          if (error || lecture_sections == null) {
            console.log(
              "<ERROR> Getting secions for lecture with ID:",
              lecture_id
            );
            res.json(error);
          } else {
            ...
            Course.findById(
              ...
              (error, lecture_course) => {
                if (error || lecture_course == null) {
                  ...
                  console.log(
                    "<SUCCESS> Getting lecture with sections and course for lecture ID:",
                    lecture_id
                  );
                  res.json(lecture);
                }
...
lectureRoutes.post("/end_early", (req, res) => {
  let now = new Date();
  Lecture.findByIdAndUpdate(req.body.lecture_id, { end_time: now }, function(
    err,
    lecture
  ) {
    if (err || !lecture) {
      console.log(
        "<ERROR> Ending lecture early for lecture with ID: " +
          req.body.lecture_id
      );
      res
        .status(404)
      ...
      console.log(
        "<SUCCESS> Ending lecture early for lecture with ID: " +
          req.body.lecture_id
      );
      res.json(lecture);
    }
